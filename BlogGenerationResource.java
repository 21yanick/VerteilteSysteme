...
public class BlogGenerationResource {
    ...
    public Response generateBlog(GeneratedBlogDTO blogRequest) {
        try {
            ...
            // Zusätzliche KI-Metadaten (können später in Phase 3 erweitert werden)
            Map<String, String> aiMetadata = new HashMap<>();
            aiMetadata.put("templateType", templateType);
            aiMetadata.put("audience", params.get("audience"));
            aiMetadata.put("tone", params.get("tone"));
            aiMetadata.put("length", params.get("length"));
            
            // Blog speichern (diese Methode müsste in der Blog-Entity hinzugefügt werden)
            //blog.setAiMetadata(aiMetadata);
